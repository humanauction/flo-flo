...
def _validate_max_count(max_count: object) -> str | None:
    if isinstance(max_count, bool):
        return "Invalid max_count: bool is not allowed; provide an integer"
    ...
    if max_count < 1 or max_count > MAX_SCRAPE_COUNT:
        return (
            f"Invalid max_count: must be between 1 and {MAX_SCRAPE_COUNT}"
        )
